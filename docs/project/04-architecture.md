...
| `/equipment` | `EquipmentPage` (`:30`) | да (`:15`) | Каталог с поиском, фильтром, пагинацией. **Состояние — в query (с11):** `q`, `type`, `subtype`, `status`, `page`, `item` |
...
| `list-draft:new` | **24 ч** | `lists/api.ts` (ключ `cacheKeys.ts`) | Черновик несохранённого списка. С с11 у записи спрашивают и `touchedAt` — `readListDraftMeta` (`lists/api.ts`) кормит карточку черновика в реестре и плашку восстановления. Мета читается БЕЗ гейта по TTL, поэтому спрашивать её можно только про черновик, который вернул `readListDraft` |
...
|---|---|---:|---|
| `fetchEquipment` | `equipment/api.ts` | `.range(from, to)`, `pageSize` 50 (`:11`) или 8 на мобильном (`:12`, выбор `:16-18`) | Нет. Честная серверная пагинация с `count: 'exact'` (`:89`). На телефоне страница 1…N тянется пачкой — см. «Накопление на телефоне» |
...
### Накопление на телефоне вместо постраничности (с11)

На мобильной раскладке постраничной навигации у каталога больше нет: 186 страниц с
двумя стрелками не давали посмотреть «что у нас вообще есть». Вместо неё — кнопка
«Показать ещё N», а номер страницы в адресе меняет смысл: не «какую страницу
показываем», а «сколько накопили».

Строки при этом остаются **чистой функцией** от фильтров и этого числа:
`EquipmentPage.tsx` собирает `pagesToLoad` = 1…N и тянет их одним `Promise.all`,
склеивая результат. Инкрементальный `append` в стейт выглядит дешевле, но даёт
двойную вставку в StrictMode и хвост от прошлого фильтра, если тот сменился, пока
запрос летел. Цена пересчёта близка к нулю: страницы по 8 строк, все кроме последней
приходят из кэша — `bypassCache` ставится только последней странице, а кнопка
«Обновить» (`reloadKey`) обходит кэш у всех.

Первый кадр из кэша рисуется **либо целиком, либо никак**: если хотя бы одна из
страниц 1…N в кэше отсутствует, склейка «две из кэша, третья пусто» показала бы
