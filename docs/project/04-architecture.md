...
Живой код — только `src/react` (**36 файлов, 6748 строк**). Дерево чистое: легаси-Vue
удалён из рабочей копии в с4 (CLAUDE.md, правило 7), и `src/` — это ровно `src/react/`.

Числа выросли против с4 (было 23 файла и 5083 строки) по трём причинам, и ни одна из
...
двух, и подросли сами фичи. `styles.css` при этом **похудел** с 889 до 779 — распил CSS
с4 снял мёртвые классы. Ни один файл больше не стоит за порогом ~800.

---

## 1. Дерево `src/react`

Числа получены `find src/react -type f | xargs wc -l`; сумма — 6748, файлов — 36
(35 `.ts`/`.tsx` на 5969 строк плюс `styles.css`).

| Файл | Строк | За что отвечает |
|---|---:|---|
| `styles.css` | 779 | Вся дизайн-система одним файлом: токены, сетка, компоненты, мобильные брейкпойнты. Импортируется один раз в `main.tsx:7` |
...
| `features/lists/ListsPage.tsx` | 551 | Реестр списков (серверные поиск, фильтр и страницы), карточка-drawer со сводкой, история этапов, дефицит, переходы статусов, удаление |
| `features/lists/api.ts` | 539 | Единственная точка доступа к `equipment_lists` / `reservation_status_history` и четырём RPC списков. Здесь же серверная пагинация, legacy-фолбэк (раздел 9) и черновик редактора |
| `features/equipment/api.ts` | 409 | Единственная точка доступа к `equipment` / `equipment_movements` и двум RPC. Здесь же нормализация `tracking_mode` / `inventory_code` |
...
| `features/equipment/EquipmentCreatePage.tsx` | 297 | Форма создания единицы: два режима учёта, проверка дубля серийника (отказ проверки блокирует сохранение), подсказки из таксономии |
| `features/equipment/EquipmentPage.tsx` | 278 | Каталог: поиск с дебаунсом, фильтр статуса, пагинация с зажимом номера страницы, префетч следующей |
...
| `lib/persistentCache.ts` | 231 | Кэш со scope пользователя (Map + localStorage), TTL, поколения инвалидации, дедупликация параллельных загрузок, LRU-обрезка. Разобран в разделе 7 |
| `app/App.tsx` | 200 | Роутер, гейт сессии с сохранением исходного пути, `AppShell` (сайдбар + прогрев), `RouteBoundary`, два скелетона |
...
| `lib/date.ts` | 19 | Календарный день строкой `YYYY-MM-DD` без `toISOString()` (Ташкент — UTC+5) |
...
| Было | Стало |
|---|---|
| `EquipmentPage.tsx` 512 | `EquipmentPage.tsx` 278 + `EquipmentDrawer.tsx` 309 + `format.ts` 12 |
...
Оба таймера с сетью гасят ошибки: `.catch(() => undefined)` на `App.tsx:93` и `:105`.
...
`RouteBoundary` (`App.tsx:180-182`) — это `<Suspense fallback={<RouteLoader/>}>{children}</Suspense>`
...
3. Пользователь кликает по разделу, который ещё не прогрелся, — срабатывает
   ...
   раздел 12), поэтому правка кода приложения больше не меняет их хэши — окно, в
   котором старая вкладка промахивается, сузилось до чанков самих страниц.
...
Единственное, что с5 к этому добавил, — **читаемость стека, когда до него добрались
руками**: `build.sourcemap: true` (`vite.config.ts:29`), и `.map` уезжают на прод рядом
с чанками. Сборщика ошибок это не заменяет, мотив записан в самом конфиге (`:28`).
...
Признака «сеть недоступна» в интерфейсе нет. В связке с `persistentCache.ts:198-204`
(при ошибке загрузчика возвращается прежнее значение, ошибка до UI не доходит) это даёт
плохой вариант: сеть лежит, а пользователь видит вчерашние данные и уверен, что смотрит
склад. **Две оговорки с с5, которые срезали худшую половину этого:** ошибки
«разлогинило / прав нет» больше не подменяются старым значением, а летят наружу
(`:201`), и синхронный первый кадр больше не рисует просроченную запись —
`readCachedQuery` сверяет `expiresAt` (`:112-116`). То есть подмена осталась только
для сетевых сбоев и только внутри TTL. Подробнее — раздел 7.
...
- `App.tsx:56` — то же для `argo:sidebar-collapsed`.

Третье место появилось в с5 и в `try/catch` тоже не обёрнуто:
`sweepLegacyEntries` (`persistentCache.ts:50-60`) перебирает `window.localStorage`
целиком при первой установке scope, то есть на пути входа.
...
|---|---|---|
| `App.tsx:93` | Прогрев каталога и списков | Ничего; просто нет ускорения |
| `App.tsx:105` | Прогрев редактора и картинок | Ничего |
| `EquipmentPage.tsx:96` | Префетч следующей страницы каталога | Ничего; переход «вперёд» просто не мгновенный |
| `lists/api.ts:394` | Прогрев состава карточки списка | Ничего; состав догрузится при открытии деталей |
| `EquipmentCreatePage.tsx:75` | `fetchEquipmentTaxonomy()` в форме создания | Пустые подсказки типа/подтипа без объяснения |
| `persistentCache.ts:198-204` | Сетевая ошибка при наличии кэша **в пределах TTL** | Данные из кэша без признака устаревания |
| `persistentCache.ts:104-106`, `:135-137`, `:162-164` | Порча/переполнение localStorage | Ничего |
| `AuthProvider.tsx:46-50` | Отказ `getSession()` | Считается «нет сессии» → редирект на `/login` |
...
**Фактические размеры** (`npm run build`, 2026-08-20):

| Чанк | Размер | gzip |
|---|---:|---:|
| `react-vendor` | 230.83 kB | 73.77 kB |
| `supabase` | 113.06 kB | 31.30 kB |
| `index` (свой каркас) | 17.75 kB | 6.76 kB |
| `ListEditorPage` | 33.53 kB | 11.06 kB |
| `ListsPage` | 26.78 kB | 8.97 kB |
| `EquipmentVisual` | 21.59 kB | 7.42 kB |
| `EquipmentPage` | 20.57 kB | 6.77 kB |
| `xlsxExport` | 17.62 kB | 5.29 kB |
| `EquipmentCreatePage` | 14.20 kB | 4.76 kB |
| CSS | 67.68 kB | 13.26 kB |
