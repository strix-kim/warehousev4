...
4. Файла нет. Правило `vercel.json:38` (`{"source": "/(.*)", "destination": "/"}`)
   применяется после проверки файловой системы, поэтому промах отдаёт не 404,
   а `index.html` со статусом 200 и `Content-Type: text/html`.
5. Браузер пытается разобрать HTML как ES-модуль → ошибка парсинга
   («Failed to fetch dynamically imported module» / unexpected token `<`).
...
Заголовок `Cache-Control: immutable` на `/assets/(.*)` (`vercel.json:10-13`) верен для
существующих файлов и на этот сценарий не влияет — проблема в удалённых, а не в
закэшированных.
...
а не выписка из базы, и `AUDIT-2026-08-19.md:36` в том же репозитории называет 223.
...
`AUDIT-2026-08-19.md` построчно не сверялся. Из проверяемого: его пункт про «основной
...
4. **Фактический размер бандла и его нарезка.** Требует `npm install` + `npm run build`;
   `node_modules` отсутствует. Утверждение `AUDIT-2026-08-19.md` о 354 КБ / 109 КБ gzip
   не проверено.
