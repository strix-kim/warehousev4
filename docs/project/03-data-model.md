...
**Габариты схемы на 2026-08-23 (с17):** 9 таблиц, 32 политики `public` + 2 на
`storage.objects`, 11 триггеров, 18 функций (`public` + `private`), 38 индексов,
1 Storage-бакет. Baseline (`00000000000000_baseline_remote_schema.sql`) снят в с2
и описывает **стартовую точку**, а не сегодняшнее состояние: поверх него легли
18 миграций. Переснимать его нужно только после большой ручной правки в дашборде.
...
| `public.employees` | `.from()`: select/insert/update, **DELETE не дан никому** | база сотрудников (с17): карточка с паспортными данными |
...
- **Директор компании в машинах НЕ хранится** (бриф просит его для шапки
  экспортного списка): он — строка `employees` с должностью «Директор», второй
  экземпляр ФИО+телефона нарушил бы один источник правды. Экспорт (роадмап)
  достанет его из `employees`.
