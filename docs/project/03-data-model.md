...
## 14. Залы: матрица распределения (с20, слот наёма — с21)
...
или слот наёма) и общий `position_catalog`. Решения, не выводимые из кода:

- **Ячейка = максимум одна запись**: UNIQUE `hall_assignments_cell_key
  (position_id, hall_id)`. Запись — либо человек, либо слот «Наём»
  (миграция `hired_operator_slots`, с21): `employee_id` nullable +
  `is_external` + CHECK `(employee_id is null) = is_external`. Три состояния
  ячейки: пустая («не решено» — записи НЕТ, вакансии-записи упразднены с20),
  человек, слот («решено: берём внешнего, имени нет» — по нему считается
  счётчик найма). «Слот только в операторской строке» база НЕ проверяет —
  правило в UI (backlog).
- **FK `employee_id` → employees — `on delete cascade`** (с21): планы —
  черновики, ячейки ушедшего из реестра освобождаются. До с21 был `set null`,
  который при NOT NULL с20 ронял удаление занятого сотрудника невнятной ошибкой.
