...
проверкой: это не гонка, а отсутствие ограничения (`02-features` §5.1).
...
RLS включён на всех семи таблицах. Все политики адресованы роли `authenticated`.
...
1. **Версия карточки сверяется.** Клиент шлёт `p_expected_updated_at`; расхождение
   отменяет правку целиком, ни одно поле не записано, код `40001`
   (`serialization_failure`). `null` значит «сверить нечем».
...
переименования модели одна позиция показывалась бы двумя строками (`02-features` §10).
