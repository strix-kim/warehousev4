...
export function useHallPlanEditor(planId: string | undefined) {
  ...
  useEffect(() => {
    ...
    // Справочник сотрудников уходит В ТОТ ЖЕ МОМЕНТ, что и пачка ниже, а не после
    // неё: от плана он не зависит вовсе (planId ему не нужен), и ожидание ready
    // ставило его вторым кругом сети — пикер клетки успевали открыть раньше, чем
    // список доезжал. Своего Promise здесь нет намеренно: загрузка живёт в
    // собственном состоянии (candidatesState), и её отказ не имеет права закрыть
    // план — как и отказ справочника позиций ниже.
    loadCandidates()
